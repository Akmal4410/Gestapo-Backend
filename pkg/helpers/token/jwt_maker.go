...
// CreateRefreshToken create a token for specific userName and duration
func (maker *JWTMaker) CreateRefreshToken(userID, userType string) (string, error) {
	mySigningKey := []byte(maker.secretKey)
	payload := NewRefreshPayload(userID, userType)
	jwtToken := jwt.NewWithClaims(jwt.SigningMethodHS256, payload)
	return jwtToken.SignedString(mySigningKey)
